...
uint8_t state;    //系统状态
uint8_t mode;     //控制模式
uint8_t start_stop_F1;   //控制F1的采集和停止
uint8_t start_stop_F2;   //控制F2的采集和停止
uint16_t PF=1000;     //频率参数变量
uint32_t compare1;    //捕获得到F1的计数值
uint32_t compare2;    //捕获得到F2的计数值
uint32_t frq1;        //F1的频率
uint32_t frq2;       //F2的频率
  ...
  switch(state)
	{
		case 0:
		  ...
		  sprintf(buf3,"     F1=%d ",frq1);
		  LCD_DisplayStringLine(Line3,(uint8_t*)buf3);
    	sprintf(buf1,"     F2=%d        ",frq2);
...
//		{
//		UAR_flag=0;
//		sprintf((char*)buf,"receive:%s\r\n",RX_BUFF);
//		HAL_UART_Transmit_DMA(&huart1,buf,20);
//		}
		LCD_show();
		key=KEY_scan();
		KEY_deal_pro(key);
